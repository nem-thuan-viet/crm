...
function columnTotal(column) {
  ...
  if (!sum) return 'đ0'
  // Kanban tải theo trang: `column.data` chỉ là số thẻ ĐÃ tải, còn nhãn bên cạnh
  // đếm `all_count` của cả cột. Cột 50 thương vụ mới tải 20 thẻ mà hiện tổng trần
  // thì đọc thành "cả cột được ngần này" — thiếu tiền mà nhìn như đủ. Thêm dấu +
  // để nói đúng: còn nữa, chưa cộng hết.
  const chuaDu = (column.column?.count ?? column.data?.length ?? 0) < (column.column?.all_count ?? 0)
  return 'đ' + Math.round(sum).toLocaleString('vi-VN') + (chuaDu ? '+' : '')
}
